...
var allowedLiterals = map[string][]string{
	// Flag defaults, overridden by the chart or the e2e manifest templates.
...
// TestNoNewHardcodedInstallLayoutInChart holds the chart to a stricter bar:
// it renders every name, so it has no reason to mention the canonical
// namespace or a SPIFFE identity at all.
func TestNoNewHardcodedInstallLayoutInChart(t *testing.T) {
	root := repoRoot(t)

	var offenders []string
	err := filepath.WalkDir(filepath.Join(root, "charts", "substrate"), func(path string, d os.DirEntry, err error) error {
		if err != nil {
			return err
		}
		if d.IsDir() || (!strings.HasSuffix(path, ".yaml") && !strings.HasSuffix(path, ".tpl")) {
			return nil
		}
		...
		for i, line := range strings.Split(string(data), "\n") {
			if strings.HasPrefix(strings.TrimSpace(line), "#") {
				continue // comments may name the defaults they explain
			}
			if suspect(line) {
	...
	if len(offenders) > 0 {
		t.Errorf("the chart must render names from the release, never hardcode the canonical layout:\n  %s",
