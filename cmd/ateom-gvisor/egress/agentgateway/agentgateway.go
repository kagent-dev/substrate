...
var _ egress.Provider = Provider{}

func (p Provider) Name() string {
	return "agentgateway"
}

func (p Provider) CapturePorts() egress.CapturePorts {
...
func (p Provider) NewGateway(ctx context.Context) (egress.Gateway, error) {
	return NewManager(ctx, p.BinaryPath, p.PodUID, p.ReapLock)
}
...
var _ egress.Gateway = (*Manager)(nil)
...
func NewManager(ctx context.Context, binaryPath, podUID string, reapLock *sync.RWMutex) (*Manager, error) {
	manager := &Manager{
		binaryPath: binaryPath,
		podUID:     podUID,
		reapLock:   reapLock,
		configPath: ateompath.EgressAgentgatewayConfigPath(podUID),
		tlsDir:     ateompath.EgressAgentgatewayTLSDir(podUID),
	}
...
func (m *Manager) ApplyPolicy(ctx context.Context, defaultNamespace string, policy *egresspb.EgressPolicy) error {
	...
	if err := m.writeConfig(ctx, defaultNamespace, policy); err != nil {
		return err
	}
	if !PolicyNeedsAgentgateway(policy) {
...
func (m *Manager) writeConfig(ctx context.Context, defaultNamespace string, policy *egresspb.EgressPolicy) error {
	...
	cfg, err := renderEgressAgentgatewayConfig(ctx, m.secret, defaultNamespace, m.tlsDir, policy)
	if err != nil {
		return err
	}
...
func PolicyNeedsAgentgateway(policy *egresspb.EgressPolicy) bool {
	...
	for _, rule := range policy.GetAllow() {
		...
		if egressRuleHasTransparentTLSRoute(rule) {
			return true
		}
...
func egressRuleHasTransparentTLSRoute(rule *egresspb.EgressPolicyRule) bool {
	...
	if !hasHost || rule.GetTls().GetMode() == "Intercept" {
		return false
	}
	for _, port := range effectiveEgressPorts(rule) {
		if egressPortUsesTLSPassthrough(rule, port) {
			return true
		}
	}
	return false
...
func egressRuleHasTransparentHTTPRoute(rule *egresspb.EgressPolicyRule) bool {
	...
	for _, port := range effectiveEgressPorts(rule) {
		if isTCPEgressPort(port) && effectiveEgressPort(port) == 80 && !egressPortUsesTLSPassthrough(rule, port) {
			return true
		}
	}
	return false
...
func renderEgressAgentgatewayConfig(ctx context.Context, secrets secretResolver, defaultNamespace, tlsDir string, policy *egresspb.EgressPolicy) ([]byte, error) {
	...
	for _, rule := range policy.GetAllow() {
		...
		if httpsListener.Protocol == "TLS" && egressRuleHasTransparentTLSRoute(rule) {
...
func routesForEgressRule(ctx context.Context, secrets secretResolver, defaultNamespace, _ string, rule *egresspb.EgressPolicyRule) ([]localAgentgatewayRoute, error) {
	...
	for _, dest := range rule.GetTo() {
		...
		for _, port := range effectiveEgressPorts(rule) {
			if !isTCPEgressPort(port) || egressPortUsesTLSPassthrough(rule, port) {
				continue
			}
			backend := localAgentgatewayRouteBackend{Host: fmt.Sprintf("%s:%d", host, effectiveEgressPort(port))}
			if egressRuleRequiresBackendTLS(rule) {
				backend.Policies = &localAgentgatewayBackendPolicies{BackendTLS: backendTLSForEgressRule(rule, host)}
			}
			// TODO: Extend route rendering here if the egress API grows L7
			// policy fields such as path matches, rate limits, or header
			// handling. The current policy intentionally stays at
			// host/port/default-action enforcement.
...
func backendTLSForEgressRule(rule *egresspb.EgressPolicyRule, host string) map[string]any {
	...
	if rule.GetTls().GetMode() == "Intercept" && !rule.GetTls().GetIntercept().GetValidateUpstream() {
...
func frontendTLSForInterceptRule(ctx context.Context, secrets secretResolver, defaultNamespace, tlsDir string, rule *egresspb.EgressPolicyRule) (*localAgentgatewayFrontendTLS, error) {
	...
	certPath := filepath.Join(tlsDir, "intercept-ca.crt")
	keyPath := filepath.Join(tlsDir, "intercept-ca.key")
	if err := os.WriteFile(certPath, cert, 0o600); err != nil {
		return nil, fmt.Errorf("while writing egress intercept CA cert: %w", err)
	}
	if err := os.WriteFile(keyPath, key, 0o600); err != nil {
		return nil, fmt.Errorf("while writing egress intercept CA key: %w", err)
	}
	return &localAgentgatewayFrontendTLS{Mode: "dynamicCa", Cert: certPath, Key: keyPath}, nil
}
